...
export async function generateMetadata({
  ...
  return {
    title: `${item.name} | ${item.category} | Pleasant Cannabis Toronto`,
    description: itemData.metaDescription,
    alternates: {
      canonical: `https://www.pleasantcannabis.ca/item/${slug}`,
    },
    openGraph: {
      title: `${item.name} | Pleasant Cannabis`,
      description: itemData.metaDescription,
...
function getJsonLd(item: ItemProduct) {
  ...
  const offers: any = {
    "@type": "Offer",
    url: `https://www.pleasantcannabis.ca/item/${item.slug}`,
    priceCurrency: "CAD",
  ...
  return {
    ...
    "@type": "Product",
    name: item.name,
    image: item.image ? [item.image.startsWith('http') ? item.image : `https://www.pleasantcannabis.ca${item.image.startsWith('/') ? '' : '/'}${item.image}`] : undefined,
  ...
  return {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": item.category,
        "item": `https://www.pleasantcannabis.ca/items/${catSlug}`
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": item.name,
        "item": `https://www.pleasantcannabis.ca/item/${item.slug}`
      }
    ]
  };
}
