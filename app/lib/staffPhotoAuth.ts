...
const COOKIE_NAME = "pcb_staff_session";
...
function sessionSecret() {
  const value = process.env.PCB_STAFF_SESSION_SECRET || "";
...
export function createSessionToken(pinVersion: number, now = Date.now()) {
  return signStaffSession(sessionSecret(), pinVersion, now, Number(process.env.PCB_STAFF_PIN_BOUNDARY_HOUR || "6"));
}

export function verifySessionToken(token: string, expectedVersion: number, now = Date.now()) {
  return validateStaffSession(token, sessionSecret(), expectedVersion, now, Number(process.env.PCB_STAFF_PIN_BOUNDARY_HOUR || "6"));
}
