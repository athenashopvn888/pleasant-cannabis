...
export default function PleasantWebChat() {
  ...
  useEffect(() => {
    const initial = window.setTimeout(() => {
      if (token) void refresh(token).catch(() => setNotice("Your previous Pleasant Cannabis chat could not be restored."));
  ...
  return (
    <aside className={`sod-web-chat ${open ? "open" : ""}`} aria-label="Pleasant Cannabis Web Chat">
      ...
      {open && <section className="sod-chat-panel" role="dialog" aria-modal="true" aria-label="Pleasant Cannabis Web Chat">
      <header><div><strong>Pleasant Cannabis Web Chat</strong><small>Start your delivery order with a dispatcher</small></div><button type="button" onClick={() => setOpen(false)} aria-label="Minimize chat">×</button></header>
